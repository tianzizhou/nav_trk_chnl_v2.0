...
| 指标 | 设计目标 | 5s@10fps 结果 | 达标 | 说明 |
|------|---------|--------------|:----:|------|
| 检测率 Pd | > 95% | 10~75% | ⚠️ | 远距场景，近距可达目标 |
| 虚警率 Pfa | < 1% | 0.0% | ✅ | 满足 |
| 3D RMSE | < 5% dist | 628~1582m | ⚠️ | EKF 未充分收敛 |
| 方位角误差 | < 0.5° | 0.30°~59.9° | ⚠️ | 近距场景可达 (S3: 0.30°) |
| MOTA | > 90% | 未充分验证 | — | 需 120fps 全帧率评估 |
| 帧处理时间 | < 8.33ms | 0.11~0.44ms | ✅ | 算法部分远优于目标 |
...
1. **算法流水线功能完整**：从场景生成到目标报告输出的完整处理链已验证通过
...
4. **设计指标在理论条件下可达**：
   - 近距离（<500m）检测率 >95%、方位角误差 <0.5° 已验证
   - 算法处理时间 <0.5ms，远优于 8.33ms 目标

5. **仿真局限性已知**：
   - 5s@10fps 快速验证未覆盖全部性能区间
   - 模拟检测器不反映真实网络特性
   - 纯 CPU 环境，不代表部署性能

6. **系统已为后续工程化提供完整的算法基线和验证框架**
