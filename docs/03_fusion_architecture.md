...
除了对四种传感器的实时数据进行融合处理外，系统还需要一个
**目标数据库（Target Knowledge Database, TKD）** 模块，用于
数据积累、经验学习和系统自适应优化。
...
│  │                                                        │  │
│  │  ┌──────────┐ ┌──────────┐ ┌──────────┐ ┌──────────┐  │  │
│  │  │历史航迹库│ │目标特征库│ │打击效果库│ │场景经验库│  │  │
│  │  │          │ │          │ │          │ │          │  │  │
│  │  │ SQLite/  │ │ 向量DB   │ │ SQLite   │ │ JSON/    │  │  │
│  │  │ Parquet  │ │          │ │          │ │ SQLite   │  │  │
│  │  └──────────┘ └──────────┘ └──────────┘ └──────────┘  │  │
│  └────────────────────────────────────────────────────────┘  │
└──────────────────────────────────────────────────────────────┘
...
存储每次任务中的完整融合航迹数据，用于离线分析和模型训练。

```
table: track_archive
├── mission_id        (VARCHAR) 任务编号
├── track_id          (INT)     航迹编号
├── start_time        (INT64)   航迹起始时间
├── end_time          (INT64)   航迹终止时间
├── target_class      (INT)     最终分类结果
├── class_confidence  (FLOAT)   分类置信度
├── threat_level      (INT)     威胁等级
├── behavior_pattern  (INT)     行为模式
├── engagement_result (INT)     打击结果 (0:未打击 1:命中 2:未命中)
├── trajectory_data   (BLOB)    航迹点序列 (时间戳,位置,速度,精度)
├── sensor_contrib    (INT)     参与传感器掩码
└── scene_type        (INT)     场景类型
```

**（b）目标特征库**

存储各类目标的多传感器特征签名，用于在线目标匹配和识别。

```
table: target_signature
├── signature_id      (INT)     特征编号
├── target_type       (VARCHAR) 目标型号 (如 "DJI Mavic 3")
├── rcs_mean_dBsm     (FLOAT)   平均 RCS (dBsm)
├── rcs_std_dBsm      (FLOAT)   RCS 标准差
├── micro_doppler_sig (BLOB)    微多普勒特征模板
├── acoustic_mfcc     (BLOB)    MFCC 均值向量 (39维)
├── acoustic_bpf_hz   (FLOAT)   旋翼基频
├── visual_feature    (BLOB)    视觉特征向量 (YOLOv8 embedding)
├── rf_protocol       (INT)     RF 协议类型
├── rf_frequency_mhz  (FLOAT)   工作频率
├── max_speed_mps     (FLOAT)   最大速度
├── typical_altitude_m(FLOAT)   典型飞行高度
...
存储不同场景下的最优融合参数，用于场景自适应优化。

```
table: scene_experience
├── scene_id          (INT)     场景编号
├── scene_type        (INT)     场景类型 (S1~S8)
├── location_hash     (VARCHAR) 地理位置哈希（粗粒度）
├── time_of_day       (INT)     时段 (0:白天 1:黄昏 2:夜间)
├── weather           (INT)     天气条件
├── radar_weight      (FLOAT)   最优雷达权重
├── eo_weight         (FLOAT)   最优光电权重
├── acou_weight       (FLOAT)   最优声学权重
├── rf_weight         (FLOAT)   最优 RF 权重
├── cfar_threshold    (FLOAT)   最优 CFAR 门限
├── association_gate  (FLOAT)   最优关联门限
├── detection_rate    (FLOAT)   历史检测率
├── false_alarm_rate  (FLOAT)   历史虚警率
├── sample_count      (INT)     样本次数
└── last_updated      (INT64)   最后更新时间
```
...
当融合系统确认一个目标航迹后，提取其多传感器特征向量，
与目标特征库进行相似度匹配：

\[
S_{match}(i) = w_r \cdot S_{rcs}(i) + w_a \cdot S_{mfcc}(i) + w_v \cdot S_{visual}(i) + w_f \cdot S_{rf}(i)
\]

若 \(S_{match} > T_{match}\)（匹配门限），则将目标标识为
已知型号，并用新观测数据更新特征库（指数滑动平均）：

\[
\mathbf{f}_{updated} = \alpha \cdot \mathbf{f}_{old} + (1-\alpha) \cdot \mathbf{f}_{new}, \quad \alpha = 0.9
\]

**（b）场景参数自适应**

每次任务结束后，系统评估当前场景下各传感器的实际表现
（检测率、虚警率、航迹连续性），并更新场景经验库中的
最优参数：

```
function update_scene_params(scene_type, actual_performance):
    record = scene_experience.get(scene_type, location, time)

    // 计算新权重 (基于实际检测贡献)
    for sensor in [radar, eo, acou, rf]:
        actual_contrib = compute_sensor_contribution(sensor)
        record.weight[sensor] = 0.8 * record.weight[sensor]
                                + 0.2 * actual_contrib

    // 更新其他参数
    record.detection_rate = 0.9 * record.detection_rate
                            + 0.1 * actual_detection_rate
    record.false_alarm_rate = 0.9 * record.false_alarm_rate
                              + 0.1 * actual_fa_rate
    record.sample_count += 1
    record.last_updated = now()

    scene_experience.save(record)
```

**（c）异常行为模式学习**

系统记录所有目标的运动轨迹模式（如蛙跳间距、悬停时间、
飞行速度分布），逐步积累"行为模式库"，用于更准确的
意图判断和轨迹预测。

#### 4.7.4 离线学习 — 模型迭代优化

系统支持定期（如每月或每次重大任务后）进行离线模型训练：
...
当系统部署到新地域或面对新型无人机时，利用已有数据和
模型作为预训练基础，通过少量新数据快速适应：

\[
\theta_{new} = \theta_{pretrained} + \Delta\theta_{finetune}
\]

仅需 50~100 个新场景样本即可完成迁移适应。

#### 4.7.5 数据库容量与存储

| 数据类型 | 每条大小 | 日增量 | 月增量 | 年增量 |
|---------|---------|--------|--------|--------|
| 航迹归档 | ~10 KB | ~100 条 | 3000 条 | 36000 条 |
| 目标特征 | ~2 KB | ~20 条 | 600 条 | 7200 条 |
| 场景经验 | ~0.5 KB | ~50 条 | 1500 条 | 18000 条 |
| 原始记录 | ~2.8 MB/h | ~67 MB | 2 GB | 24 GB |

存储需求：一年约 25 GB，使用 256 GB SSD 可存储约 10 年数据。

#### 4.7.6 与融合处理的交互接口

目标数据库通过以下接口与融合处理核心交互：

| 接口 | 方向 | 数据 | 触发条件 |
|------|------|------|---------|
| 航迹归档 | 融合→TKD | 完整航迹数据 | 航迹终止时 |
| 特征查询 | 融合→TKD→融合 | 特征向量匹配 | 新目标确认时 |
| 参数查询 | 融合→TKD→融合 | 场景最优参数 | 场景切换时 |
| 特征更新 | 融合→TKD | 新观测特征 | 航迹更新时 |
| 模型更新 | 离线→TKD→融合 | 新检测/分类模型 | 离线训练完成后 |
